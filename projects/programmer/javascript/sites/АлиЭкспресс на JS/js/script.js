    ...
    loadContent('js/db.json', () => {
        ...
        goodsBtn.forEach(function (btn, i) {
            btn.addEventListener('click', () => {
                let item = products[i].cloneNode(true), // Копируем весь код карточки товара
        ...
        function sliceTitle() { // Функция для обрезки title
            titles.forEach(function (item) {
                if (item.textContent.length < 65) {
                    return;
                } else {
                    const str = item.textContent.slice(0, 65) + '...'; // ES5
        ...
        function showConfirm() {
            ...
            function frame() { // Создаём анимацию
                if (counter === 10) {
                    clearInterval(id);
